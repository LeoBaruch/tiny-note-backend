...
impl From<sqlx::Error> for NoteError {
    fn from(e: sqlx::Error) -> Self { NoteError::Db(e) }
}

pub async fn create_note(state: &AppState, user_id: Uuid, req: CreateNoteRequest) -> Result<Note, NoteError> {
    let note_id = Uuid::new_v4();
    sqlx::query("INSERT INTO notes (id, user_id, title, content, tags, created_at, updated_at) VALUES (?, ?, ?, ?, ?, CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+08:00'), CONVERT_TZ(UTC_TIMESTAMP(), '+00:00', '+08:00'))")
        .bind(note_id)
        .bind(user_id)
        .bind(&req.title)
        .bind(&req.content)
    ...
    let note = sqlx::query_as::<_, Note>("SELECT id, user_id, title, content, tags, created_at, updated_at FROM notes WHERE id = ?")
        .bind(note_id)
        .fetch_one(&state.db)
        .await?;
    Ok(note)
...
pub async fn list_notes(state: &AppState, user_id: Uuid, tag: Option<String>, keyword: Option<String>) -> Result<Vec<Note>, NoteError> {
    let mut q = String::from("SELECT id, user_id, title, content, tags, created_at, updated_at FROM notes WHERE user_id = ?");
    if tag.is_some() { q.push_str(" AND tags LIKE ?"); }
    if keyword.is_some() { q.push_str(" AND (title LIKE ? OR content LIKE ?)"); }
    q.push_str(" ORDER BY updated_at DESC");

    let mut query = sqlx::query_as::<_, Note>(&q).bind(user_id);
    if let Some(t) = tag.as_ref() { query = query.bind(format!("%{}%", t)); }
    if let Some(k) = keyword.as_ref() { query = query.bind(format!("%{}%", k)).bind(format!("%{}%", k)); }
...
pub async fn get_note(state: &AppState, user_id: Uuid, note_id: Uuid) -> Result<Note, NoteError> {
    let note = sqlx::query_as::<_, Note>("SELECT id, user_id, title, content, tags, created_at, updated_at FROM notes WHERE id = ? AND user_id = ?")
        ...
        .await?;
    match note { Some(n) => Ok(n), None => Err(NoteError::NotFound) }
}

pub async fn update_note(state: &AppState, user_id: Uuid, note_id: Uuid, req: UpdateNoteRequest) -> Result<Note, NoteError> {
    // Fetch current
...
pub async fn delete_note(state: &AppState, user_id: Uuid, note_id: Uuid) -> Result<(), NoteError> {
    let res = sqlx::query("DELETE FROM notes WHERE id = ? AND user_id = ?")
        ...
        .execute(&state.db)
        .await?;
    if res.rows_affected() == 0 { return Err(NoteError::NotFound); }
    Ok(())
}